/*
  ...
  Copyright (C) 2010 TWiki Contributors http://twiki.org
  All Rights Reserved.
...
(function () {
    ...
    tinymce.create('tinymce.plugins.TWikiButtons', {
        ...
        ** twikibuttons_cursoridletime param */
        nodeChangeEventFrequency: null,

        init: function (ed, url) {
            this.formats = ed.getParam('twikibuttons_formats');
            ...
            jQuery.each(this.formats, function (key, value) {
                ed.plugins.twikibuttons.format_names.push(key);
            });
            /* Register TWiki formats with TinyMCE's formatter, which isn't
               available during plugin init */
            ed.onInit.add(function (editor) {
                this.plugins.twikibuttons._registerFormats(editor, this.plugins.twikibuttons.formats);

                this.plugins.twikibuttons._contextMenuVerbatimClasses(editor);
            });
        ...
        createControl: function (name, controlManager) {
            if (name === 'twikiformat') {
                return this._createTWikiFormatControl(name,
                    controlManager, this);
            }
        ...
        _createTWikiFormatControl: function (name, controlManager, plugin) {
        ...
        _setupFormatCommand: function (ed, formats) {
            ed.addCommand('twikibuttonsFormat', function (ui, formatname) {
        ...
        _setupAttachButton: function (ed, url) {
            ed.addCommand('twikibuttonsAttach', function () {
                ...
                if (null !== TWikiTiny.twikiVars.TOPIC.match(
                    /(X{10}|AUTOINC[0-9]+)/)) {
       ...
       _setupIndentButton: function (ed, url) {
            ed.addCommand('fwindent', function () {
                if (this.queryCommandState('InsertUnorderedList') ||
                    ...
                    if (node) {
                        ...
                        div = dom.create('div', { 'class' : 'twikiIndent'});
            ...
            ed.addButton('fwindent', {
                title: 'twikibuttons.indent_desc',
                cmd: 'fwindent',
        ...
        _setupExdentButton: function (ed, url) {
            ed.addCommand('fwexdent', function () {
                var dom = ed.dom, selection = ed.selection,
                    node = dom.getParent(selection.getStart(), dom.isBlock),
                    p;
                if (node && dom.hasClass(node, 'twikiIndent')) {
                    p = node.parentNode;
                    while (node.firstChild) {
            ...
            ed.onNodeChange.add(function(ed, cm, n, co, ob) {
                var dom = ed.dom, selection = ed.selection,
                    node = dom.getParent(selection.getStart(), dom.isBlock),
                    state = (node && dom.hasClass(node, 'twikiIndent')) ||
            ...
            ed.addButton('fwexdent', {
                title: 'twikibuttons.exdent_desc',
                cmd: 'fwexdent',
        ...
        ** no need to update it yet again.
        ...
        _updateButtonState: function (ed, cm, node, collapsed) {
            var selectedFormats = ed.formatter.matchAll(
                    ed.plugins.twikibuttons.format_names),
                listbox = cm.get(ed.id + '_twikiformat');
        ...
        _contextMenuVerbatimClasses: function (ed) {
            ...
            ed.plugins.twikibuttons.recipe_names = ['bash','cplusplus','csharp','css','delphi','html','java','js','lotusscript','php','sql','tml'];
            for (i = 0; i < ed.plugins.twikibuttons.recipe_names.length; i += 1) {
                key = ed.plugins.twikibuttons.recipe_names[i];
            ...
            jQuery.each(recipes, function (key, value) {
                ed.plugins.twikibuttons.recipe_names.push(key);
            });
            if (ed && ed.plugins.contextmenu) {
                ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
                    ...
                    if (el.nodeName === 'PRE' && el.className.indexOf('TMLverbatim') !== -1) {
                        selectedFormats = ed.formatter.matchAll(ed.plugins.twikibuttons.recipe_names);
                        if (selectedFormats.length > 0) {
                        ...
                        jQuery.each(recipes, function (name, format) {
                            sm.add({title :  name , cmd : 'twikiVerbatimClass', value : name });
                        });
                        sm.add({title : 'none', cmd : 'twikiVerbatimClass', value : ' ' });
                    }
                });
            }

            ed.addCommand('twikiVerbatimClass', function(ui, val) {
                // First, remove all existing formats.
                jQuery.each(recipes, function (name, format) {
                    ed.formatter.remove(name);
                });
    ...
    // Register plugin
    tinymce.PluginManager.add('twikibuttons', tinymce.plugins.TWikiButtons);
})();
