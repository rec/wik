...
# **REGEX EXPERT**
# A server may have aliases. This is to specify a regular expression matching
# possible server names. This regular expression suppose to span from the
# protocol part up to the port number e.g. qr{http://(?:www\.)?domain\.com}
# This is used to detect recursive INCLUDE loop such as:
# INCLUDE{http://THIS_SERVER/cgi-bin/view/THIS_WEB/THIS_TOPIC}
# Here are some notes:
#    * Please be careful not to have capturing groups in the value.
#      That's why the example above has (?:www\.) instead of (www\.).
#    * The value can either be a regex value (qr{...}) or a string.
#    * When you set a string value, please don't forget to escape \
#      -- e.g. '\\.'  matches a dot while '\.' matches any non-newline
#      character.
...
#   the browser, so it requires enabling {UseClientSessions} to track client
#   sessions.
# </li><li>
...
#   not require client sessions, but works best if {UseClientSessions} is
#   enabled.
# </li></ol>
...
# **STRING 200 EXPERT**
# This affects how isAdmin() result is cached. If each web has its own
# admin group and $users->isAdmin($cUID, $topic, $web) is $web sensitive,
# this needs to be set 'web'. Otherwise it needs to be 'site'.
...
# **STRING 100 EXPERT**
# Specify forbidden actions of users in the following format.
#   CanonUserName1: script1, script2;
#   canonUserName2: script3, script3;
#   ...
# Please read TWiki.TWikiAccessControl#Forbid_certain_users_to_do_certa for
# details.
...
# SCRIPT can still be used in the HEAD section, though. <br /> <b>Note:</b>
# This may prevent the TopMenuSkin and some other extensions from functioning
# correctly.
...
# As such, when you view a topic, the web browser sees P tags not explicitly
# closed, which messes up the DOM tree.
# If you put "<p></p>" in place of "<p />", the DOM tree gets better and
# more predictable. To avoid unexpected consequences, TWiki keeps putting
# "<p />" by default. If {OpenClosePTags} is true, "<p></p>" is put instead.
$TWiki::cfg{OpenClosePTags} = $FALSE;
...
# **BOOLEAN EXPERT**
# Disable XSS-Protection implemented in some browsers.
# When a user saves a topic page that contains any JavaScript, the result
# view page (redirected from the last POST) is subject to the XSS protection,
# where the edited JavaScript is not executed, until the page is reloaded and
# displayed again.
# It is inconvenient when some JavaScript in a TWiki topic is incrementally
# edited again and again during development.
# This configuration option adds 'X-XSS-Protection: 0' to the HTTP response
# header, so that the JavaScript is executed even when the page is edited and
# saved.
$TWiki::cfg{DisableXSSProtection} = $FALSE;
...
# **BOOLEAN**
# Even if you don't use a locale, you may want to use non-ASCII characters in
# attachment file names. If so, make this true
$TWiki::cfg{AllowI18NFileName} = $FALSE;
...
# **STRING 100**
# %<nop>LANG% value is calculated from {Site}{Locale} but you may want to
# explicitly specify. In such a case, set $TWiki::cfg{Site}{Lang}.
# $TWiki::cfg{Site}{Lang}
...
# **NUMBER EXPERT**
# TWiki::Render::summariseChanges() calls TWiki::Merge::simpleMerge(), which
# calls Algorithm::Diff::traverse_balanced(), which may cause out-of-memory
# with a topic bigger than two megabytes. The limit may vary depending on the
# version of Algorithm::Diff and other factors.
# This is to set the limit of the text size TWiki::Render::summariseChanges()
# handles in bytes. Setting 0 means no limit.
# Please read TWikibug:Item7562 for details.
...
# **REGEX EXPERT**
# If a web name matches this, it's regarded invalid.
# Please read TWikibug:Item7838 for details.
...
# **NUMBER**
# Number of top viewers to show in statistics topic
...
# **NUMBER**
# Number of webs viewed most to be listed on the "Webs Viewed" column
# in the site statistics topic
$TWiki::cfg{Stats}{SiteTopViews} = 0;

# **NUMBER**
# Number of webs updated most to be listed on the "Webs Updated" column
# in the site statistics topic
$TWiki::cfg{Stats}{SiteTopUpdates} = 0;

# **NUMBER**
# Number of top viewers to show in site statistics topic
$TWiki::cfg{Stats}{SiteTopViewers} = 10;

# **NUMBER**
# Number of top contributors to show in site statistics topic
...
# **BOOLEAN EXPERT**
# The statistics script updates WebStatistics of webs and Main.SiteStatistics.
# Setting this pparameter true causes WebStatisticsYYYY and
# Main.SiteStatisticsYYYY to be updated where YYYY is the current year.
# Please refer to TWiki.TWikiSiteTools#WebStatisticsYYYY for details.
$TWiki::cfg{Stats}{TopicPerYear} = $FALSE;

# **STRING EXPERT**
# The regular expression of web names to be excluded in !WebStatistics update.
# Please read %SYSTEMWEB%.TWikiSiteTools#ExcludedWebRegex for details.
$TWiki::cfg{Stats}{ExcludedWebRegex} = '';

# **STRING EXPERT**
# Whether or not showing affiliation breakdown of views, saves, and uploads.
# For this to work, the user mapping handler must have the
# getAffiliation($cUID) object method. Please read
# TWiki.TWikiSiteTools#AffiliationBreakdown for more information.
$TWiki::cfg{Stats}{Breakdown} = $FALSE;

# **NUMBER EXPERT**
# In the affiliation breakdown in web statistics, at most how many affiliations
# are listed
$TWiki::cfg{Stats}{TopAffiliation} = 10;

# **NUMBER EXPERT**
# In the affiliation breakdown in site statistics, at most how many affiliations
# are listed
...
# **STRING 120**
# List of exceptional domains (comma-separated) to which links should *not* be
# converted to the external link format.
# If you want to maintain the list of domains in some topic page, specify
# "topic:TopicName" or "topic:WebName.TopicName" where WebName defaults to
# %SYSTEMWEB%. In the topic, the domain names should be listed in a TWiki table
# format (left-most column in each row).
...
# **STRING 20 EXPERT**
# Specifies to read user preferences before web preferences
# (the only way up to TWiki 6.0.0)
# or at the last step of preferences variable setting
# (new option from 6.0.1)
# Please read TWiki.TWikiVariables#ControllingUserLevelOverride for details
$TWiki::cfg{DemoteUserPreferences} = $FALSE;
...
# **STRING EXPERT**
# Regular expression of valid record IDs
$TWiki::cfg{Mdrepo}{RecordIDRe} = '\\w+';

# **STRING EXPERT**
# Regular expression of valid field names
...
#---++ User subwebs
# Please read TWiki.UserSubwebs for details
# **BOOLEAN 50 EXPERT**
# Whether or not to use user subwebs instead of user topics.
# You can avoid a site is cluttered by personal top level webs by employing
# user subwebs !Main/FirstLast.WebHome in place of user topics
#  !Main.FirstLast.
$TWiki::cfg{UserSubwebs}{Enabled}            = $FALSE;

# **BOOLEAN 100 EXPERT**
# You can have user subwebs ignore !Main.WebPeferences by setting this true.
$TWiki::cfg{UserSubwebs}{IgnoreParentPrefs}  = $FALSE;

# **STRING EXPERT**
# If you have your user subweb, your user preferences are read from
# the topic specified by the following parameter in your user subweb.
...
$TWiki::cfg{Plugins}{TWikiSheetPlugin}{Enabled} = 1;
