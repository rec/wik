...
sub login {
    ...
    if( $loginName ) {
        ...
        # Item7538: TWiki:Codev.LoginWithTwoStepAuthentication
        my $twoStepAuthManager = $TWiki::cfg{TwoStepAuthManager} || 'none';
        unless( $this->{twoStepAuthManager} || $twoStepAuthManager eq 'none' ) {
            # two-factor authentication manager is specified, but not yet initialized
            eval "require $twoStepAuthManager";
            die $@ if $@;
            $this->{twoStepAuthManager} = $twoStepAuthManager->new( $twikiSession );
        }

        if( $this->{twoStepAuthManager} && $validation ) {
            # present second challenge on two-factor auth
            $tmpl = $this->{twoStepAuthManager}->secondStepAuth( $loginName, $origurl );
            $validation = 0 if( $tmpl );

        } elsif( $this->{twoStepAuthManager} && $accessCode ) {
            # received access code, verify it for second challenge on two-factor auth
            $banner = $this->{twoStepAuthManager}->verifyAuth( $loginName, $accessCode );
