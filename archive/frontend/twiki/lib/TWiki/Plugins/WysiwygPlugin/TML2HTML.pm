...
Convertor class for translating TML (TWiki Markup Language) into
HTML
...
use TWiki::Plugins::WysiwygPlugin::Handlers;

use strict;
use warnings;

# Used by _takeOutBlocks/_putBackBlocks
our $BLOCKID = 0;
our $OC      = "<!--\0";
our $CC      = "\0-->";
...
# is common for href attributes to contain variables. Users should
...
sub convert {
    ...
    my $disabled =
      TWiki::Plugins::WysiwygPlugin::wysiwygEditingDisabledForThisContent(
        $content);
    if ($disabled) {
      ...
      # encode the content verbatim-style, so that the user has uncorrupted HTML
        ...
        $content = CGI::div(
            { class => 'WYSIWYG_WARNING twikiBroadcastMessage' },
            TWiki::Func::renderText(
                TWiki::Func::expandCommonVariables( <<"WARNING" ) ) )
...
WARNING
        ...
        my $tagsToProtect = TWiki::Func::getPreferencesValue(
        ...
        my $tagBlocksToProtect =
          TWiki::Func::getPreferencesValue('WYSIWYGPLUGIN_PROTECT_TAG_BLOCKS')
          || 'script,style';
...
# Parse and convert variables. If we are not using span markers
...
sub _processTags {
    ...
    # Variables at the start of a line must *stay* at the start of a line.
    # The newline preceding the mcro must be preserved.
    # This is important for variables like %SEARCH that can emit
    # line-oriented TML.
    #
...
sub _getRenderedVersion {
    ...
    # Protect anchors
    $text =~
s/^($TWiki::regex{anchorRegex})/$this->_liftOut("\n$1", 'PROTECTED')/gems;
...
s/((^|(?<=[-*\s(]))$TWiki::regex{linkProtocolPattern}:[^\s<>"]+[^\s*.,!?;:)<])/$this->_liftOut($1, 'LINK')/geo;
    ...
    # Wrap tables with variables before or after them in a <div>,
    ...
    my $tableAndMacrosDivStart = '<div class="twikiTableAndMacros">';
    my $tableAndMacrosDivEnd   = '</div><!--twikiTableAndMacros-->';
    while (@divIndexes) {
    ...
    my @result = ();
    my $spi = TWiki::Func::getContext->{SUPPORTS_PARA_INDENT};
    ...
    foreach my $line ( split( /\n/, $text ) ) {
            ...
            s/^((\t|   )+)\$\s(([^:]+|:[^\s]+)+?):\s/<dt> $3 <\/dt><dd> /o )
        {
            ...
            && $line =~ s/^((\t|   )+): /<div class='twikiIndent'> /o )
        {

            # Indent pseudo-list
            $this->_addListItem( \@result, '', 'div', 'class="twikiIndent"',
                $1 );
            $inList = 1;
        }
    ...
    unless (
        TWiki::isTrue( TWiki::Func::getPreferencesValue('NOAUTOLINK') ) )
    {
        my $removed = {};
 ...
 # Need to also include protected content marker as part of start wikiword delim
        ...
        $text =~
s/$startww(($TWiki::regex{webNameRegex}\.)?$TWiki::regex{wikiWordRegex}($TWiki::regex{anchorRegex})?)/$this->_liftOutSquab($1,$1)/geom;
    ...
    if ( $text =~ /$tagsBeforeFirstTablePattern/o ) {
        $text = '<p class="twikiDeleteMe">&nbsp;</p>' . $text;
    }
...
# This wraps the contents in a paragraph, and avoids the split.  It will be removed on save.
    $text =~
s/(<blockquote.*?>)(.*?)(<\/blockquote)/$1<p class="twikiDeleteMe">$2<\/p>$3/sgi;
...
sub _liftOutSquab {
    ...
    if (   $class eq 'TMLlink'
        && $url =~ m/\s/
        && $url =~ m/^$TWiki::regex{linkProtocolPattern}:/ )
    {
    ...
    my $startww = qr/$WC::STARTWW|(?<=$TT2)/;
    if ( $url =~
m/$startww(($TWiki::regex{webNameRegex}\.)?$TWiki::regex{wikiWordRegex}($TWiki::regex{anchorRegex})?)/
        && $url eq $text )
    {
...
sub _processTableRow {
    ...
    $row =~ s/(\|\|+)/'colspan'.$TWiki::TranslationToken.length($1)."\|"/geo
      ;                      # calc COLSPAN
    my $colCount = 0;
    my @cols     = ();
    my $span     = 0;
    ...
    my $rowspanEnabled = TWiki::Func::getContext()->{'TablePluginEnabled'};

    foreach ( split( /\|/, $row ) ) {
        my $attr = {};
        $span = 1;
        if (s/colspan$TWiki::TranslationToken([0-9]+)//) {
            $span = $1;
            $attr->{colspan} = $span;
        }
        s/^\s+$/ &nbsp; /o;
...
sub _getNamedColour {
    my ( $name, $t ) = @_;
    my $epr = TWiki::Func::getPreferencesValue($name);
...
sub _takeOutIMGTag {
    ...
    # Expand selected variables in IMG tags so that images appear in the
    # editor as images
    $text =~
