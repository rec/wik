...
sub _isProtectedByAttrs {
    ...
    foreach my $attr ( keys %{ $this->{attrs} } ) {
        ...
        return $attr
          if TWiki::Plugins::WysiwygPlugin::Handlers::protectedByAttr(
            $this->{tag}, $attr );
    }
    return 0;
}
...
sub _convertIndent {
    ...
    return $t unless TWiki::Func::getContext->{SUPPORTS_PARA_INDENT};
    ...
    my $p = $this;
    while ( $p = $p->{parent} ) {
        if ( $p->{tag} eq 'div' && $p->hasClass('twikiIndent') ) {
            $indent .= $WC::TAB;
        }
    }
    $t =~ s/^$WC::WS*//s;
...
sub _isConvertableIndent {
    ...
    return 0 unless TWiki::Func::getContext->{SUPPORTS_PARA_INDENT};

    return 0 if ( $this->_isProtectedByAttrs() );

    return $this->{tag} eq 'div' && $this->hasClass('twikiIndent');
}
...
sub _isConvertableTable {
    ...
    my $rowspan = undef;
    $rowspan = [] if TWiki::Func::getContext()->{'TablePluginEnabled'};
...
# The limit on the number of digits will ensure that the numbers are reasonable.
    ...
    # Remove all other (non twiki) classes
    if ( defined $atts{class} && $atts{class} !~ /twiki/ ) {
        delete $atts{class};
    }
