...
"$TWiki::Plugins::EditTablePlugin::web.$TWiki::Plugins::EditTablePlugin::topic"
	...
	$headerRowCount = $query->param('etheaderrows')
	  if ( $query->param('etheaderrows') );
	$footerRowCount = $query->param('etfooterrows')
	  if ( $query->param('etfooterrows') );
...
s/^(\s*)\|(.*)/handleTableRow( $1, $2, $tableNr, $isNewRow, $rowNr, $doEdit, $doSave, $theWeb, $theTopic )/eo;
        ...
        if ( $doEdit || $doSave ) {
            ...
            if ( $headerRowCount > 0 && $headerRowCount <= @bodyRows ) {
            ...
            if ( $footerRowCount > 0 && $footerRowCount <= @bodyRows ) {
                @footerRows = @rows;    # clone
                splice @footerRows, 0, - $footerRowCount;

                # remove the footer rows from the body rows
                splice @bodyRows, - $footerRowCount;
            }
		...
		$theRowNr = $newBodyRowNr + scalar @lines
		  if ( $headerRowCount > scalar @lines );
    ...
    if ($doSave) {
        my $error = '';
        if( $query && $query->request_method() && $query->request_method() !~ /^POST$/i ) {
...
sub handleTableStart {
    ...
    my $ctx = TWiki::Func::getContext();
    my $viewUrl = TWiki::Func::getScriptUrl( $theWeb, $theTopic,
                      ($ctx->{authenticated} ? 'view' : 'viewauth') )
...
sub handleTableRow {
    ...
    _decodeFormatTokens( $text ) if ( !$doSave && !$doEdit );
    ...
    unless ($breakLock) {
        ...
        if ($oopsUrl) {
            ...
            if ( $lockUser ne $loginUser ) {
                ...
                # use viewauth instead of view if necessary
                my $ctx = TWiki::Func::getContext();
...
sub _encodeValue {
    return if( $_[0] eq '' );
    # convert <br /> markup to linebreak entity (for textareas)
    $_[0] =~ s/<br *\/?>/\x00/gos;
    $_[0] =~ s/([[\x01-\x09\x0b\x0c\x0e-\x1f"%&'*<=>@[_\|\s])/'&#'.ord($1).';'/ge;
    $_[0] =~ s/\x00/&#10;/gos;
}
