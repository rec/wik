# NOTE: This is copied from http://search.cpan.org/~deti/Proc-Daemon-0.14/lib/Proc/Daemon.pod
# It is identical to Proc::Daemon but uses the TWiki namespace
...
package TWiki::Plugins::BackupRestorePlugin::ProcDaemon;

use strict;
use POSIX();

$TWiki::Plugins::BackupRestorePlugin::ProcDaemon::VERSION = '0.14';


################################################################################
# Create the Daemon object:
# my $daemon = TWiki::Plugins::BackupRestorePlugin::ProcDaemon->new( [ %Daemon_Settings ] )
#
...
# TWiki::Plugins::BackupRestorePlugin::ProcDaemon::Init( [ { %Daemon_Settings } ] )
...
################################################################################
sub Init {
    my TWiki::Plugins::BackupRestorePlugin::ProcDaemon $self = shift;
    my $settings_ref = shift;


    # Check if $self has been blessed into the package, otherwise do it now.
    unless ( ref( $self ) && eval{ $self->isa( 'TWiki::Plugins::BackupRestorePlugin::ProcDaemon' ) } ) {
        $self = ref( $self ) eq 'HASH'
              ? TWiki::Plugins::BackupRestorePlugin::ProcDaemon->new( %$self )
              : TWiki::Plugins::BackupRestorePlugin::ProcDaemon->new();
    }
