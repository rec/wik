...
sub getDiskInfo {
    ...
    my $cacheKey = "$site:$web" . ((!$web && $diskID) ? ":$diskID" : '');
    ...
    if ( my $mdrepo = $session->{mdrepo} ) {
        if ( $web ) {
            if ( my $webRec =
                 $mdrepo->getRec('webs', TWiki::topLevelWeb($web))
            ) {
                $diskID = $webRec->{disk} || '';
    ...
    $cache->{$cacheKey} = [$dataDir, $pubDir, $diskID];
...
sub saveFile {
    ...
    my $FILE;
    my $tmpName = $name . '.' . $$; # Item7760
    open($FILE, '>', $tmpName ) ||
      throw Error::Simple( 'RCS: failed to create file '.$tmpName.': '.$! );
    binmode($FILE ) ||
      throw Error::Simple( 'RCS: failed to binmode '.$tmpName.': '.$! );
    print $FILE $text;
    close($FILE) ||
      throw Error::Simple( 'RCS: failed to create file '.$tmpName.': '.$! );
    rename($tmpName, $name) or do { # Item7760
        my $nameNoDir = $name;
        $nameNoDir =~ s:^.*/::;
        throw Error::Simple( 'RCS: failed to rename file '.$tmpName.
            ' to '.$nameNoDir.': '.$! );
    };
    return;
}
