...
sub loadKey {
    ...
    my @key;
    while( $key =~ /^(-----BEGIN ((RSA|DSA) PRIVATE KEY)-----\n(?:(.*?\n)\n)?.*?^-----END (?:RSA|DSA) PRIVATE KEY-----$)/msgo ) {
        ...
        unless( $passkey ) {
            push @key, status => [ ERROR => "Encrypted keyfile not supported for this item" ];
            next;
        }
        unless( defined $password && length $password ) {
            push @key, status => [ ERROR => "$passkey must be specified; see $passkey" ];
            next;
        }
        unless( $h{'DEK-Info'} ) {
            push @key, status => [ ERROR => "Corrupt file: missing DEK-Info encryption header\n"];
            next;
        }
        unless( $h{'DEK-Info'} =~ /([\w-]+),([[:xdigit:]]+)$/ && $decryptable{$1} ) {
            push @key, status => [ WARNING => "$1 encryption is not supported" ];
            next;
        }
        my( $encryption, $iv ) = ( $1, $2 );

        push @key, $decryptable{$encryption}->( $format, $pem, $encryption, $iv, $passkey, $password );
    } continue {
        my %k;
...
sub check {
    ...
    my $password;
    if( $passkey ) {
        $password = eval "\$TWiki::cfg$passkey";
        return $this->ERROR( "Can't evaluate current value of $passkey: $@" ) if( $@ );
    }

    my( $errors, @keys ) = loadKey( $value, $passkey, $password );

    return $this->ERROR( "No key in file: " . $keys[0] ) if( $errors );
    ...
    my $notes = sprintf( "\
Key Information: %s %s %s key", ucfirst $key->{encrypted}, $key->{type}, $key->{view} );
    ...
    if( @keys ) {
        $warnings .= "File contains " . @keys . " additional keys.  These will not be used and should be removed.";
    }
