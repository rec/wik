...
sub buildNewTopic {
    ...
    if ( !$topicExists && $store->webExists( $webName.'/'.$topic ) ) {
        throw TWiki::OopsException(
            'attention',
...
WARN
    ...
    if ( $saveCmd &&
         ! $session->{users}->isAdmin( $session->{user}, $topic, $web )
    ) {
        throw TWiki::OopsException(
            'accessdenied',
            def => 'only_group',
    ...
    if ( $session->{rcsFileNull} ) {
        throw TWiki::OopsException(
            'attention',
            def => 'prior_versions_lost',
            web => $web,
            topic => $topic,
            params => "past revisions are lost."  );
    }
