...
our $VERSION = '$Rev: 30543 (2018-07-16) $';
...
sub initContrib {
    ...
    $TWiki::cfg{MailerContrib}{CustomUserGroupNotations} ||= qr/\n/;
       # the default value guaranteed not to match anything
}
...
#        print STDERR "**** ERROR mailnotifier cannot find web $web\n";
    ...
    my $saveWebName = $twiki->{webName}; #Item7248
    $twiki->{webName} = $web;
...
sub _sendChangesMails {
    ...
    foreach my $email ( keys %{$changeset} ) {
        ...
        my $error = TWiki::Func::sendEmail( $mail,
                        $TWiki::cfg{MailerContrib}{SendEmailTries} || 5 );
