...
sub isMandatory {
    ...
    return ( $parameters && $parameters->{mandatory} ) ? 1 : 0;
}
...
sub renderForDisplay {
    ...
    unless( $attrs->{showhidden} ) {
        my $fp = $this->{parameters};
        return '' if( $fp && $fp->{hidden} );
    }
    ...
    $format =~ s/\$attributes/$this->attributesAsText()/ge;
    ...
    # Item7169: $value moved to bottom to avoid expansion of other tokens
...
sub attributesAsText {
    ...
    my $key;
    my @ss;
    foreach $key ( sort keys %$fp ) {
        unless( $key =~ /^(_ERROR|_RAW)$/ ) {
