...
sub new {
    ...
    if( $web && $topic ) {
        if ( $type =~ /^USER / ) {
            my $session = $prefs->{session};
            my $denied = _commaListToHashRef(
                $prefs->getPreferencesValue( 'DENYUSERPREFERENCES' ));
            if ( !defined($denied) || ref $denied ) {
                $this->{DENIED} = $denied if ( $denied );
                my $allowed = _commaListToHashRef(
                    $prefs->getPreferencesValue( 'ALLOWUSERPREFERENCES' ));
                if ( ref $allowed ) {
                    $this->{ALLOWED} = $allowed;
                }
                # if ALLOWUSERPREFERENCES == all, $this->{ALLOWED} is not set
            }
            else {
                # DENYUSERPREFERENCES == all
                # in which case, not creating the object at all
                return undef;
            }
        }
...
sub insert {
    ...
    return 0
        if ( $this->{CONTEXT}->isFinalised( $key ) ||
             $this->{DENIED} && $this->{DENIED}{$key} ||
             $this->{ALLOWED} && !$this->{ALLOWED}{$key} );
    ...
    $value =~ s/%$key\b/%<nop>$key/g;   # Item7569: prevent obvious recursion
    if( $type eq 'Local' ) {
