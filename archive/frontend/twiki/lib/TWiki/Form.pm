...
sub new {
    my( $class, $session, $web, $form, $def, $options ) = @_;
    $options ||= {};
    ...
    unless ( $form =~ m:[./]: ) {
        ...
        if ( $formPath && $formPath =~ /\S/ ) {
            if ( $formPath =~ /%/ ) {
                $formPath = $session->handleCommonTags($formPath,
    ...
    unless( $this ) {
        ...
        unless ( $def ) {
            $dontCache = $options->{suppressValsEval};
            ...
            $this->{fields} = _parseFormDefinition( $this, $meta, $text, $options );
        ...
        $session->{forms}->{"$web.$form"} = $this unless ( $dontCache );
    }
    ...
    foreach my $line ( split( /\n/, $text ) ) {
        ...
        if( $inBlock && $line =~ s/^\s*\|\s*// ) {
            ...
            $vals = $this->{session}->handleCommonTags( $vals, $this->{web}, $this->{topic}, $meta )
              if( $vals =~ /%[A-Za-z]/ && !$options->{suppressValsEval} );
...
sub _link {
    ...
    if( $store->topicExists( $web, $topic ) ) {
        ...
        $string = $this->{session}->handleCommonTags( $string, $web, $topic, $meta )
          if( $string =~ /%[A-Za-z]/ );
    ...
    foreach my $fieldDef ( @{$this->{fields}} ) {
        ...
        if(! $title && !$fieldDef->isEditable() ) {
            ...
            unless( defined( $value )) {
                ...
                if( defined( $dv )) {
                    $dv = $session->handleCommonTags( $dv, $web, $topic, $meta )
                      if( $dv =~ /%[A-Za-z]/ );
