...
sub _createWeb {
    ...
    $session->writeLog('createweb', $newWeb, $baseWeb);
...
sub rename {
    ...
    if ( $newWeb eq $trashWebName &&
        ...
        if( $attachment ) {
            ...
            my $parent = $store->getTopicParent( $oldWeb, $oldTopic );
            my( $parentWeb, $parentTopic );
            if( defined $parent ) {
                ( $parentWeb, $parentTopic ) =
                  $session->normalizeWebTopicName( $oldWeb, $parent );
            }
            if( $parentTopic &&
...
sub _renameweb {
    ...
    unless ( $session->{users}->canRenameWeb($oldWeb, $newWeb) ) {
...
sub _saveSettings {
    ...
    TWiki::UI::checkAccess( $session, $web, $topic, 'CHANGE', $session->{user} );
