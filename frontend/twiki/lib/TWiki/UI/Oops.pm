...
sub oops {
    ...
    if( ! $tmplData ) {
        ...
        # Item7486: $session->handleCommonTags(...) used to be called here,
        $n = 1;
        ...
        $tmplData =~ s/%PARAM\d+%//g; # Item7686
        if ( $tmplData =~ /%REDIRECTTOURL%/ ) {
            my $redirDest = $session->_getRedirectUrl() || '%WEB%.%TOPIC%';
            $tmplData =~ s/%REDIRECTTOURL%/$redirDest/g;
        }
    ...
    $session->writeCompletePage( $tmplData, undef, undef, $status );
}
