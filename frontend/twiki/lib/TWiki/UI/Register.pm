...
sub _buildConfirmationEmail {
    ...
    my $loginAs = $data->{LoginName} || $data->{WikiName};
    my $caller  = $session->{users}->getWikiName($session->{user});
    if ($caller eq $TWiki::cfg{DefaultUserWikiName}) {
        $caller = $data->{WikiName};
    }
    ...
    $templateText =~ s/%LOGINAS%/$loginAs/g;
    $templateText =~ s/%CALLER%/$caller/g;
...
sub _getDataFromQuery {
    ...
    foreach( $query->param() ) {
        if (/^(Twk)([0-9])(.*)/) {
            ...
            my $value = join(', ', @values); #deal with multivalue fields like checkboxes
            $form->{name} = $name;
            $form->{value} = $value;
            if ( $name eq 'Password' ) {
