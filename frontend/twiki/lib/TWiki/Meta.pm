...
sub topicTitle {
    ...
    my $title;
    $session->{plugins}->dispatch('topicTitleHandler', $session,
                                  $this->{_web}, $this->{_topic}, \$title);
...
sub renderFormFieldForDisplay {
    ...
    if( $fname ) {
        require TWiki::Form;
        my $form = new TWiki::Form( $this->{_session}, $this->{_web}, $fname,
                                    undef, {suppressValsEval => 1} );
            # The last argument is for Item6082/Item7146.
            # The original fix of Item6081 introduced a bug and later
            # reverted but the bug re-emerged as Item7146.
            # Value options are not needed for display hence suppress
            # variable evaluation in the values. This is to avoid infinite
            # recursion which happens in a situation described in
