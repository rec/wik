...
HERE
        ...
        if( $text =~ /[\n\r]\-\-\-+(\++)/s ) {
            my $level = $1;
            $level =~ s/\+/\\\+/go;
            # add slide number to headings
            my $slideNum = 0;
            $text =~ s/([\n\r]\-\-\-+$level\!*) ([^\n\r]+)/"\n<a name=\"Slide_" .
              ++$slideNum . "\"><\/a>$1 Slide $slideNum: $2"/ges;
        }
        $text = "$textPre \n#StartPresentation\n"
...
sub renderSlideNav
{
    ...
    my $viewUrl = TWiki::Func::getViewUrl($theWeb, $theTopic);
    my $style = 'style="border: none;"';
    if( $theButtons =~ /f/ ) {
        # first slide button
        if( $theButtons =~ / f/ ) {
            $text .= "&nbsp;";
        }
        $text .= "<a href=\"$viewUrl$qstring#GoSlide1\" $style>"
    ...
    if( $theButtons =~ /p/ ) {
        ...
        $text .= "<a href=\"$viewUrl$qstring#GoSlide$prev\" $style>"
    ...
    if( $theButtons =~ /n/ ) {
        ...
        $text .= "<a href=\"$viewUrl$qstring#GoSlide$next\" $style>"
    ...
    if( $theButtons =~ /l/ ) {
        ...
        $text .= "<a href=\"$viewUrl$qstring#GoSlide$theMax\" $style>"
