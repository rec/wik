...
sub handleTabPane {
  ...
  my $requires = $tabPaneId; # add previous ID as required, e.g. enforce sorting
  $tabPaneId = 'jqTabPane'.($tabPaneCounter++); # new ID
  my $select = $params->{select} || '';

  $select =~ s/[^\d]//go;
  $select ||= 1;

  TWiki::Func::addToHEAD( $tabPaneId, <<"EOS", $requires );
<script type="text/javascript">
jQuery(function(\$) {
  \$("#$tabPaneId").tabpane({select:$select});
});
</script>
EOS

  return " <div class=\"jqTabPane\" id=\"$tabPaneId\">";
}
