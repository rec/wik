# Package TWiki::Plugins::BackupRestore::CaptureOutput
#
# Package renamed from IO::CaptureOutput, by Simon Flack & David Golden
# http://search.cpan.org/~dagolden/IO-CaptureOutput-1.1102/lib/IO/CaptureOutput.pod
#
# Portions copyright 2004, 2005 Simon Flack.
# Portions copyright 2007, 2008 David Golden. All rights reserved.
#
...
package TWiki::Plugins::BackupRestorePlugin::CaptureOutput;
use strict;
...
sub _capture (&@) { ## no critic
    ...
    if ( !defined $output || $output != \undef ) { 
        $capture_out = TWiki::Plugins::BackupRestorePlugin::CaptureOutput::_proxy->new(
            'STDOUT', $output, undef, $output_file
        );
    }
    if ( !defined $error || $error != \undef ) { 
        $capture_err = TWiki::Plugins::BackupRestorePlugin::CaptureOutput::_proxy->new(
            'STDERR', $error, ($should_merge ? 'STDOUT' : undef), $error_file
        );
    }

    # now that output capture is setup, call the subroutine
    # results get read when TWiki::Plugins::BackupRestorePlugin::CaptureOutput::_proxy objects go out of scope
    &$code();
}
...
package TWiki::Plugins::BackupRestorePlugin::CaptureOutput::_proxy;
...
TWiki::Plugins::BackupRestorePlugin::CaptureOutput - capture STDOUT and STDERR from Perl code, subprocesses or XS
...
= SYNOPSIS
    ...
    use TWiki::Plugins::BackupRestorePlugin::CaptureOutput qw(capture qxx qxy);
