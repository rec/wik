...
sub checkAccess {
    ...
    my $authenticated = $twiki->inContext( 'authenticated' );
    my $script = $twiki->{request}->action;

    if ( $authenticated ) {
        unless ( $twiki->{forbidUserAction} ) {
            $twiki->{forbidUserAction} =
                _fuaStrToHash($TWiki::cfg{ForbidUserAction});
        }
        if ( my $actions = $twiki->{forbidUserAction}{$twiki->{user}} ) {
            my $negative;
            if ( $actions =~ /^!/ ) {
                $actions =~ s///;
                $negative = 1;
    ...
    unless( $authenticated || $TWiki::cfg{LoginManager} eq 'none' ) {
...
sub modifyHeader {
    ...
    # If a crypto-token enabled form is present, inhibit caching

    if( $this->{_crypttokenPresent} ) {
        $hopts->{expires} = '-1d';
        $hopts->{'cache-control'} = 'no-cache';
    }
...
sub _SESSION_VARIABLE {
    ...
    # Item7543: Do not allow set/get/clear session variables starting with underscore
    return '' if( $name =~ /^_/ );
