...
sub saveTopic {
    ...
    if( !$options->{ignorePermissions} && $user &&
          !$this->{session}->security->checkAccessPermission
...
sub saveAttachment {
    ...
    try {
        ...
        if( !$opts->{ignorePermissions} && $user &&
              !$this->{session}->security->checkAccessPermission
        ...
        if ( $opts->{stream} ) {
            ...
            unless ( $attrs ) {
                throw TWiki::OopsException (
                    ...
                    web    => $web,
                    topic  => $topic,
                    def    => 'changing_prop_of_nonexistent_file',
                    params => [ $attachment ],
                );
            }
            $attrs->{name} = $attachment;
        ...
        if( $opts->{updatefield} ) {
            ...
            if( defined $fieldName ) {
                my $value = $opts->{updateformat} || '$filename';
                $value =~ s/\$filename/$attachment/;
                $fieldName->{value} = $value;
                $meta->putKeyed( 'FIELD', $fieldName );
            }
        }

        $this->saveTopic( $user, $web, $topic, $text, $meta, {ignorePermissions=>1} );
...
# If $filter->{canmoveto} or $filter->{cancopyto} is true, further trimming is
# conducted.
# If $filter->{canmoveto} is true, webs residing on a different disk from the
# current web are eliminated. Plus webs matching
# $TWiki::cfg{WEBLIST}{canmovetoExclude} are eliminated.
# If $filter->{cancopyto} is true, webs matching
# $TWiki::cfg{WEBLIST}{cancopytoExclude} are eliminated.
#
# $TWiki::cfg{WEBLIST}{canmovetoExclude} and
# $TWiki::cfg{WEBLIST}{cancopytoExclude} are for trash webs and their rotation.
# The "Trash" web is not suitable as a [%SYSTEMWEB%.TWikiScripts#copy][copy]]
# script destination. If %SYSTEMWEB%.UsingMultipleDisks is in effect,
# !Trashx1x, !Trashx2x, ... are there in addition to Trash.
# As such, $TWiki::cfg{WEBLIST}{cancopytoExclude} has a regex instead of a
# string.
# You may rotate trash web(s) to prevent them from getting bloated:
# !Trash1, !Trash2, ..., !Trashx1x1, !Trashx1x2, ...
# They are not suitable as move destination.
sub _filterWritable {
    my ($this, $webListRef, $filter) = @_;
    $filter ||= {};
    my @result;
    my $session = $this->{session};
    my $theDiskID = $TWiki::cfg{MultipleDisks} ?
        ($this->getDiskInfo($session->{webName}))[2] : '';
        # $theDiskID is not needed if $filter->{canmoveto} is false.
        # So the value is wasted in that case.
        # But it's done this way because setting it only if $filter->{canmoveto}
        # is true may lead to bugs in the future when $theDiskID is used 
        # even if $filter->{canmoveto} is false.
        # $theDiskID looks to have a correct value regardless of
        # $filter->{canmoveto}
    for my $i ( @$webListRef ) {
...
sub getListOfWebs {
    my( $this, $filterStr, $web, $depth ) = @_;
    $filterStr ||= '';
    $web ||= '';
    $web =~ s#\.#/#g;
    my %filter;
    for my $i ( qw(canmoveto cancopyto template user public allowed writable) )
    {
        $filter{$i} = 1 if ( $filterStr =~ /\b$i\b/ );
    }
    # The canmoveto filter is for rename/move destination list.
    # The cancopyto filter is for copy destination list.

    my @webList = _getSubWebs( $this, $web, \%filter, $depth );

    if ( $filter{user} ) {
        @webList = grep { !/(?:^_|\/_)/, } @webList;
    } elsif( $filter{template} ) {
        @webList = grep { /(?:^_|\/_)/, } @webList;
    }
    ...
    if ( $TWiki::cfg{ReadOnlyAndMirrorWebs}{SiteName} ) {
        if ( $filter{canmoveto} || $filter{cancopyto} || $filter{writable} ) {
            @webList = $this->_filterWritable(\@webList, \%filter);
        }
    }
...
sub createWeb {
    ...
    my $excludeTopic =
        TWiki::Search::_makeTopicPattern($opts->{EXCLUDETOPIC});
    my $excludeException =
        TWiki::Search::_makeTopicPattern($opts->{EXCLUDEEXCEPTION});
    delete $opts->{EXCLUDETOPIC};
    delete $opts->{EXCLUDEEXCEPTION};
    if ( $excludeTopic ) {
        if ( $excludeException ) {
            @topicList = grep { /$excludeException/ || !/$excludeTopic/ }
                @topicList;
        }
        else {
...
sub searchMetaData {
    ...
    my $searchVal = '';
    my $attrFormat = $params->{format} || '$topic';
    my $attrSeparator = $params->{separator}; # use default of SEARCH
    ...
    my $text = $this->{session}->search->searchWeb(
            search        => $searchVal,
            format        => $attrFormat,
