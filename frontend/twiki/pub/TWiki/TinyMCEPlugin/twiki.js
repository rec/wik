TWikiTiny.install();jQuery(document).ready(function($){$("#save").closest('form').bind('submit',function(event){if(($('#cancel:focus')).length){return true;}
...
var ok=true;var els=twiki.getElementsByClassName(document,'twikiMandatory','select');for(var j=0;j<els.length;j++){var one=false;for(var k=0;k<els[j].options.length;k++){if(els[j].options[k].selected){one=true;break;}}
if(!one){alert("The required form field '"+els[j].name+"' has no value.");ok=false;}}
var taglist=new Array('input','textarea');for(var i=0;i<taglist.length;i++){els=twiki.getElementsByClassName(document,'twikiMandatory',taglist[i]);for(var j=0;j<els.length;j++){if(els[j].value==null||els[j].value.length==0){alert("The required form field '"+els[j].name+"' has no value.");ok=false;}}}
return ok;}
